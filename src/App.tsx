...
export default function App() {
  ...
  useEffect(() => {
    ...
    const unsubSettings = onSnapshot(collection(db, 'crm_settings'), (snap) => {
      ...
      const settingsDoc = docs.find(d => d.id === 'general_cbm');
  ...
  const calculatedData = useMemo(() => {
    ...
    const groupedArray = Object.values(groupedInventoryMap).sort((a, b) => {
      const statusWeight = { 'in_warehouse': 1, 'in_transit': 2, 'ordered': 3, 'sold': 4 };
      if (statusWeight[a.status] !== statusWeight[b.status]) return statusWeight[a.status] - statusWeight[b.status];
      return a.model.localeCompare(b.model);
    });

    return { enrichedItems, groupedInventory: groupedArray, shipmentStats, campaignStats, inWarehouseCount, totalInventoryValueILS, avgProfit: soldCount > 0 ? Math.round(totalProfit / soldCount) : 0, forecasts };
            ...
            <div className="bg-white shadow-sm border border-slate-200 rounded-lg overflow-hidden">
              <table className="min-w-full divide-y divide-slate-200 text-sm">
                ...
                <tbody className="divide-y divide-slate-200">
                  {shipments.map(s => {
                    const shippingTotalILS = (Number(s.shippingCostUSD) * Number(s.exchangeRate)) + Number(s.shippingCostILS);
                    const factoryTotalUSD = s.lines ? s.lines.reduce((acc, l) => acc + (Number(l.qty)*Number(l.unitCostUSD)), 0) : 0;
                    return (
                                ...
                                <tbody>
                                  {group.items.map(item => (
                                    <tr key={item.id} className="hover:bg-slate-50 border-t border-slate-100">
                                      ...
                                      <td className="px-3 py-2 text-left"><button onClick={(e) => { e.stopPropagation(); setEditingData(item); setIsItemModalOpen(true); }} className="text-indigo-600 bg-indigo-50 p-1 rounded"><Edit className="w-3.5 h-3.5"/></button></td>
                                    </tr>
                                  ))}
                                </tbody>
                              </table>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            <div className="p-5 border-b border-slate-100 flex justify-between items-center sticky top-0 bg-white">
              <h3 className="text-lg font-bold text-slate-800">עריכת פריט בודד במלאי: {editingData.model}</h3>
            ...
            <form onSubmit={saveItem} className="p-5 space-y-5">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-slate-700 mb-1">סטטוס פריט (נשלט ע"י המשלוח)</label>
                ...
                <div><label className="block text-sm font-medium text-slate-700 mb-1">מספר סידורי / הערה</label><input type="text" className="w-full border-slate-300 rounded-md p-2 border" value={editingData.serialNumber || ''} onChange={e => setEditingData({...editingData, serialNumber: e.target.value})} /></div>
              </div>

              <div className="grid grid-cols-2 gap-4 bg-slate-50 p-4 rounded border border-slate-100">
                <div><label className="block text-sm font-medium text-slate-700 mb-1">תאריך הגעה למחסן</label><input type="date" className="w-full border-slate-300 rounded-md p-2 text-sm bg-slate-100 text-slate-500" disabled value={editingData.arrivalDate || ''} /></div>
                <div><label className="block text-sm font-medium text-slate-700 mb-1">תאריך מכירה</label><input type="date" className="w-full border-slate-300 rounded-md p-2 text-sm" disabled={editingData.status !== 'sold'} value={editingData.saleDate || ''} onChange={e => setEditingData({...editingData, saleDate: e.target.value})} /></div>
              </div>
              ...
              <div className="bg-blue-50/50 p-4 rounded border border-blue-100">
                <label className="block text-sm font-bold text-blue-900 mb-2">שיוך לקמפיין שיווקי</label>
